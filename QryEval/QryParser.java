  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      if(Character.isDigit(queryString.charAt(0))){
        ...
        weight = Double.valueOf(p.getPopped());
...
//        p = popTerm(queryString);
//        if(p.getPopped().matches("[0-9]+\\.[0-9]++")){
//          weight = Double.valueOf(p.getPopped());
//          queryString = p.getRemaining().trim();
//        }
//      }
	...
	//  STUDENTS WILL NEED TO ADJUST THIS BLOCK TO HANDLE WEIGHTS IN HW2
        ...
        if(queryTree instanceof  QrySopWsum){
          QrySopWsum tmpQueryTree = (QrySopWsum) queryTree;
          tmpQueryTree.args_weight.add(weight);
        }
        if(queryTree instanceof  QrySopWand){
          QrySopWand tmpQueryTree = (QrySopWand) queryTree;
          tmpQueryTree.args_weight.add(weight);
        }
